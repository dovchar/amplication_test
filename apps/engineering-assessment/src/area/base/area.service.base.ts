/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { PrismaService } from "../../prisma/prisma.service";
import { Prisma, Area, Criterion } from "@prisma/client";

export class AreaServiceBase {
  constructor(protected readonly prisma: PrismaService) {}

  async count<T extends Prisma.AreaCountArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaCountArgs>
  ): Promise<number> {
    return this.prisma.area.count(args);
  }

  async findMany<T extends Prisma.AreaFindManyArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaFindManyArgs>
  ): Promise<Area[]> {
    return this.prisma.area.findMany(args);
  }
  async findOne<T extends Prisma.AreaFindUniqueArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaFindUniqueArgs>
  ): Promise<Area | null> {
    return this.prisma.area.findUnique(args);
  }
  async create<T extends Prisma.AreaCreateArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaCreateArgs>
  ): Promise<Area> {
    return this.prisma.area.create<T>(args);
  }
  async update<T extends Prisma.AreaUpdateArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaUpdateArgs>
  ): Promise<Area> {
    return this.prisma.area.update<T>(args);
  }
  async delete<T extends Prisma.AreaDeleteArgs>(
    args: Prisma.SelectSubset<T, Prisma.AreaDeleteArgs>
  ): Promise<Area> {
    return this.prisma.area.delete(args);
  }

  async getCriteria(parentId: string): Promise<Criterion | null> {
    return this.prisma.area
      .findUnique({
        where: { id: parentId },
      })
      .criteria();
  }
}
